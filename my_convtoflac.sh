...
## ToDo: check why read doesn't work; paraniod verification; chuesheet checks; info

# metaflac --import-cuesheet-from="image.cue" --set-tag-from-file="CUESHEET=image.cue" "image.flac"
...
# don't let random=random be a problem ;)
	while [ -e "$TAGS" ]; do
		TAGS=/tmp/$PROG.$RANDOM.tags
	done
	touch "$TAGS"
...
function transcode() {
	...
	if [ $EMBEDCUESHEETS ]; then
		if [ -e "$DIRECTORY/$NAME.cue" ]; then
			echo "Found cuesheet $NAME.cue" 
			CUESHEET="--cuesheet=$NAME.cue"
		else
			OLDIFS=$IFS
			IFS="
"
			CUESHEETS=`find "$DIRECTORY" -name "*\.cue" | sort`
			echo $CUESHEETS
			if [ "$CUESHEETS" ]; then
				INDEX=1
				for CUESHEET in ${CUESHEETS[@]}; do
					echo "$INDEX: $CUESHEET"
					let "INDEX++"
				done
				read CUE_NUMBER </dev/tty
				if [ $CUE_NUMBER ]; then
					echo "Using cuesheet ${CUESHEETS[(($CUE_NUMBER-1))]}"
					CUESHEET="--cuesheet=${CUESHEETS[(($CUE_NUMBER-1))]}"
				else
					echo "No cuesheet selected."
				fi
			else
				cwarn "Warning: No cuesheet found."
			fi
			IFS=$OLDIFS
	...
	if [ -n "$USEFFMPEG" ] && [ "$EXT" == "ape" -o "$EXT" == "m4a" -o "$EXT" == "shn" -o "$EXT" == "wv" ]; then
		WAVENAME="/tmp/$(basename $NAME).wav"
		[ $THREADS -gt 1 ] && QUIET='2>/dev/null'
		eval $FFMPEG -i \"$FILE\" -f wav \"$WAVENAME.wav\" $QUIET
		if [ $? -ne 0 ]; then
			cwarn "\nError: \"$FILE\" could not be converted to a FLAC file."
			rm "$WAVENAME.wav"
			exit 1
		fi
		eval $FLAC $VERIFY -$COMPRESS $OVERWRITE -o \"$NAME.flac\" \"$WAVENAME.wav\" $QUIET
		if [ $? -ne 0 ]; then
			cwarn "\nError: \"$FILE\" could not be converted to a FLAC file."
			rm "$WAVENAME.wav"
			exit 1
	...
	# Otherwise, use dedicated binaries for decoding
		...
		if [ "$EXT" == "ape" ]; then
			[ $THREADS -gt 1 ] && QUIET='2>/dev/null'
			eval $MAC \"$FILE\" - -d $QUIET | $FLAC $VERIFY -$COMPRESS $OVERWRITE -s -o "$NAME.flac" -
...
# flac allows reencoding:
			$FLAC $VERIFY -$COMPRESS $OVERWRITE -s -o "$NAME.flac" "$FILE"
		...
		# ALAC input
			...
			[ $THREADS -gt 1 ] && QUIET='-s'
			$ALAC "$FILE" | $FLAC $VERIFY -$COMPRESS $OVERWRITE $QUIET -o "$NAME.flac" -

		# Shorten input
		elif [ "$EXT" == "shn" ]; then
			[ $THREADS -gt 1 ] && QUIET='-s'
			$SHORTEN -x "$FILE" - | $FLAC $VERIFY -$COMPRESS $OVERWRITE $QUIET -o "$NAME.flac" -

		# True Audio input
		elif [ "$EXT" == "tta" ]; then
			[ $THREADS -gt 1 ] && QUIET='2>/dev/null'
			eval $TTAENC -d -o - \"$FILE\" $QUIET | $FLAC $VERIFY -$COMPRESS $OVERWRITE -s -o "$NAME.flac" -

		# WAVE input
		elif [ "$EXT" == "wav" ]; then
			[ $THREADS -gt 1 ] && QUIET='-s'
			$FLAC $VERIFY -$COMPRESS $OVERWRITE $QUIET -o "$NAME.flac" "$FILE"

		# WavPack input
		elif [ "$EXT" == "wv" ]; then
			[ $THREADS -gt 1 ] && QUIET='-q'
			$WVUNPACK $QUIET "$FILE" -o - | $FLAC $VERIFY -$COMPRESS $OVERWRITE -s -o "$NAME.flac" -
		fi
	fi
...
for FILE in ${FILES[@]}; do
	...
	if [[ "$EXT" != "ape" && "$EXT" != "flac" && "$EXT" != "m4a" && "$EXT" != "shn" && "$EXT" != "tta" && "$EXT" != "wav" && "$EXT" != "wv" ]]; then
